...
##############################################################
def get_sim_dt(client_id):
	
	if client_id != -1:
		res, sim_dt = sim.simxGetFloatingParameter(client_id,sim.sim_floatparam_simulation_time_step, sim.simx_opmode_blocking)
		#sim_dt = round(sim_dt)
		sim_dt = round(1000*sim_dt)
		#print(sim_dt,"simdt")
	return sim_dt
...
def init_remote_api_server():
	...
	sim.simxFinish(-1) #disconnecting all open connections
	client_id = sim.simxStart('127.0.0.1',19997,True,True,5000,5) #starting remote api connection at port 19997
	#print(client_id)
...
def start_simulation():
	...
	global sim_dt
	if client_id != -1:
		sim.simxSynchronous(client_id,True) #Enabling synchronous mode
		return_code = sim.simxStartSimulation(client_id, sim.simx_opmode_oneshot) #Starting simulation (non-blocking function call)
		sim.simxGetPingTime(client_id) #Getting time required for command to be sent to server, executed and sent back to fn
		sim_dt = get_sim_dt(client_id)
	else:
		return_code = sim.simx_return_initialize_error_flag #Error flag generated when simxStart isn't yet called
...
def get_vision_sensor_image():
	...
	errorcode, camhandle = sim.simxGetObjectHandle(client_id,"vision_sensor_1",sim.simx_opmode_oneshot_wait) #object handle of vision sensor
	
	
	
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,camhandle,0,sim.simx_opmode_streaming) #getting vision sensor image in non-blocking function call
	
	simulation_time = sim.simxGetLastCmdTime(client_id) #Getting simulation time of last fetched command
	sim.simxGetPingTime(client_id)
	#sim_dt = get_sim_dt(client_id) #blocking function call to ensure synchronization 
	
	
	

	while (sim.simxGetConnectionId(client_id) != -1):
	#this while loop runs while client is connected to remote api server
		try:
		
			simulation_time_2 = sim.simxGetLastCmdTime(client_id) #gets simulation time of last fetched command
			
		
			return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, camhandle, 0, sim.simx_opmode_buffer)
			#Get vision sensor image called in blocking function call
			if simulation_time_2-simulation_time>sim_dt:
				#This condition breaks the while loop after an interval of simulation_time_step
				if ((return_code == sim.simx_return_ok) and (len(image_resolution) == 2) and (len(vision_sensor_image) > 0)):
				#This condition breaks the while loop after an image is captured
					
				
					break
				else:
				#This condition continues the while loop till an image is captured
					continue
			...
			pass
		errorCode = sim.simxSynchronousTrigger(client_id) #blocking function call to server
		#makes the server call simHandleMainScript
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	array_image = np.array(vision_sensor_image,dtype=np.uint8)
	array_image.resize(image_resolution[0],image_resolution[1],3)
	transformed_image = cv2.cvtColor(array_image,cv2.COLOR_BGR2RGB)
...
def exit_remote_api_server():
	...
	##############	ADD YOUR CODE HERE	##############
	sim.simxGetPingTime(client_id) #blocking function call
	#makes exit_remote_api wait for the last command sent to arrive at server
	if sim.simxGetConnectionId(client_id) != -1:
		
		sim.simxFinish(-1) #remote api connection closed
