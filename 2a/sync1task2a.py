...
##############################################################
def step_simulation(res,vhandle,client_id):
    
    global int_signal_name = "stepcounter"
    global steps = 0
    current_step = 0
    
    _ = sim.simxSynchronousTrigger(client_id,enable)
    while steps==current_step:
        retCode,s=sim.simxGetIntegerSignal(client_id,int_signal_name,sim.simx_opmode_buffer)
        if retCode == sim.simx_return_ok:
            steps = s
    retCode,res,img=sim.simxGetVisionSensorImage(client_id,vhandle,0,sim.simx_opmode_buffer)
    img_acq_time =sim.simxGetLastCmdTime(client_id)
    return retCode,res,img
...
def init_remote_api_server():
    ...
    ##############	ADD YOUR CODE HERE	##############
    sim.simxFinish(-1) #to close all open connections
    client_id = sim.simxStart('127.0.0.1',19997,True,True,5000,5) # Connect to CoppeliaSim in blocking mode
...
def start_simulation():
    ...
    ##############	ADD YOUR CODE HERE	##############
    if client_id !=-1:
        _ = sim.simxSynchronous(client_id,True); #enabling synchronous mode
        #this is a "blocking" function call : we wait for a reply from the server
...
def get_vision_sensor_image():
    ...
    ##############	ADD YOUR CODE HERE	##############
    res, vhandle = sim.simxGetObjectHandle(client_id, "vision_sensor_1",simx.op_mode_blocking)
   
    return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,vhandle, 0,sim.simx_op_mode_streaming)
    
    #return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,vhandle, 0,sim.simx_op_mode_buffer)
    if return_code == sim.simx_return_ok:
        return_code, image_resolution, vision_sensor_image = step_simulation()
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
    ...
    ##############	ADD YOUR CODE HERE	##############
    vision_sensor_image = np.array([vision_sensor_image],dtype="uint8")
    ...
    transformed_image = cv2.flip(vision_sensor_image,1)
...
def stop_simulation():
    ...
    ##############	ADD YOUR CODE HERE	##############
    return_code = sim.simxStopSimulation(client_id,sim.simx_op_mode_blocking)
