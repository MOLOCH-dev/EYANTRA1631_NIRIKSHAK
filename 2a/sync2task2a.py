...
def init_remote_api_server():
	...
	##############	ADD YOUR CODE HERE	##############
	sim.simxFinish(-1) 											  #close any opened connection
	client_id = sim.simxStart('127.0.0.1',19997,True,True,5000,5) #starting a communication thread with server
	##################################################
...
def get_vision_sensor_image():
	...
	##############	ADD YOUR CODE HERE	##############
	#out = sim.simxAuxiliaryConsoleOpen("Debug",8,1)
	#sim.simxAuxiliaryConsolePrint(out,"Hello")
	#result,t0,t1 = sim.simxReadVisionSensor(cam)
	#xtarget = t1[5]
	#ytarget = t1[6]	
	camHandle = sim.simxGetObjectHandle(client_id,"vision_sensor_1",sim.simx_opmode_blocking)
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, camHandle, 1, sim.simx_opmode_streaming)
...
def exit_remote_api_server():
	...
	i.e. CoppeliaSim using sim.simxFinish Remote API.
