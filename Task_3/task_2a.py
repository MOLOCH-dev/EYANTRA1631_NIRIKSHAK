...
def get_vision_sensor_image(vision_sensor_handle):
	...
	global client_id, vision_sensor_image, image_resolution, return_code
	vision_sensor_image = []
	image_resolution = []
	return_code = 0
	...
	#camhandle = vision_sensor_handle
	...
	#this while loop runs while client is connected to remote api server
		try:
			...
			return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, camhandle, 1, sim.simx_opmode_buffer)
			i=i+1
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	array_image.resize(image_resolution[0],image_resolution[1],1)
	#transformed_image = cv2.cvtColor(array_image,cv2.COLOR_GRAY2RGB)
	transformed_image = cv2.flip(array_image,0)
	#transformed_image = cv2.cvtColor(array_image,cv2.COLOR_RGB2GRAY)
