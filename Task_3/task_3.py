...
##############################################################
global pex,pey,errxsum,errysum
pex=0
pey=0
errxsum=0
errysum=0
...
def constrain(angle,axis):
	if np.abs([axis])>100:
		if angle>10*np.pi/180:
			angle = 10*np.pi/180


		elif angle<-10*np.pi/180:
			angle = -10*np.pi/180
	elif np.abs([axis])<=100:
		if angle>5*np.pi/180:
			angle = 5*np.pi/180


		elif angle<-5*np.pi/180:
			angle = -5*np.pi/180
	return angle
...
def init_setup(rec_client_id):
	...
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,vision_sensor_handle,1,sim.simx_opmode_streaming) #getting vision sensor image in non-blocking function call
...
def control_logic(center_x,center_y):
	...
	"""
	global setpoint, client_id, handle_arr,trigger,pex,pey, errxsum,errysum,ogx,ogy
	...
	ogy=error_y
	error_x = pmap(error_x,-1280+set_x,1280-set_x,-1*0.1745,1*0.1745)

	error_y = pmap(error_y,-1280+set_y,1280-set_y,-1*1*0.1745,1*1*0.1745)
	errxsum = errxsum+error_x
	errysum=errysum+error_y
	#print(error_x,error_y,"exy")
	Kpx =9#6.5 #3.23 5x 4-4ex234
	Kdx=150	  #40.25
	Kpy=21 #5.5
	Kix=0
	Kdy=347	 #30 x35,55-4
	...
	if pex is None:
		...
		PIDx = Kpx*error_x + Kdx*(error_x-pex) +Kix*(errxsum)
		#print(Kpx*error_x ,Kdx*(error_x-pex),"pdx")
		PIDy = Kpy*error_y + Kdy*(error_y-pey)+Kiy*(errysum)
		#print(Kpy*error_y ,Kdy*(error_y-pey),"pdy")

	PIDx = constrain(PIDx,ogx)
	PIDy=constrain(PIDy,ogy)
	targets = [PIDy,-PIDx,-PIDy,PIDx]
