...
def get_vision_sensor_image():
	...
	while (sim.simxGetConnectionId(client_id) != -1):
		...
		try:
			...
			#print(sim.simxGetPingTime(client_id),"pingtime")
...
def send_data(client_id,maze_array):
	...
	##############	ADD YOUR CODE HERE	##############
	for i in range(0,10):
		return_code,retInts,retFloats,retStrings,retBuffer=sim.simxCallScriptFunction(client_id,'top_plate_respondable_1',6,'receiveData',maze_array[i],inputFloats,inputStrings,inputBuffer,sim.simx_opmode_blocking)
