...
def find_path(maze_array, start_coord, end_coord):
	...
	left_free = [2, 10, 6, 14, 0, 4, 12, 8]
	right_free = [11, 2, 10, 3, 0, 1, 9, 8]
	up_free = [5, 13, 0, 4, 12, 1, 9, 8]
	down_free = [7, 5, 2, 6, 3, 0, 4, 1]
	...
	while( len(frontier) > 0):
		...
		x, y = frontier.pop(0)
		wall_code = maze_array[x][y];
		# print(wall_code)
		if((x, y) == end_coord):
			end_reached = True
			break		

		if((wall_code in left_free) and ((x, y - 1) not in visited)):
		...
		if((wall_code in right_free) and ((x, y + 1) not in visited)):
			if( (x >= 0 and x < 10) and (y + 1 >= 0 and y + 1 < 10)):			
				cell = (x, y + 1)
				solution[cell] = (x, y)
				frontier.append(cell)
		...
		if((wall_code in up_free) and ((x - 1, y) not in visited)):
		...
		if((wall_code in down_free) and ((x + 1, y) not in visited)):
			if( (x + 1 >= 0 and x + 1 < 10) and (y >= 0 and y < 10)):			
				cell = (x + 1, y)
				solution[cell] = (x, y)
				frontier.append(cell)
...
def read_start_end_coordinates(file_name, maze_name):
	...
	data = json.load(f)
	maze = data[maze_name]
	start_coord = tuple(maze["start_coord"])
	end_coord = tuple(maze["end_coord"])
