...
function receiveData(inInts,inFloats,inStrings,inBuffer)
    ...
    maze_array[k]=inInts;
    --print(inInts);
    --print("maze array is ",maze_array);
    k=k+1;
    --*******************************************************
...
function generateHorizontalWalls()
	...
	--               ADD YOUR CODE HERE
	horiwall={{},{},{},{},{},{},{},{},{},{},{}}
    parent=sim.getObjectHandle("force_sensor_pw_t1_1")
    for j=0,10,1 do
        for i=1,10,1 do	
			horiwall[j+1][i]= createWall()
			sim.setObjectParent(horiwall[j+1][i],parent,false)
            sim.setObjectPosition(horiwall[j+1][i],sim_handle_parent,{-0.55+(i*0.1),0.5-j*0.1,0.08})
            sim.setObjectName(horiwall[j+1][i],"Hor_block_".. tostring(j+1).."_"..tostring(i))
		end
end
	--*******************************************************
end
...
function generateVerticalWalls()
    ...
    for j=0,10,1 do
        for i=1,10,1 do
			vertiwall[j+1][i] = createWall()
			sim.setObjectParent(wallObjectHandle,parent,false)
            sim.setObjectOrientation(wallObjectHandle,sim_handle_parent,{0,0,math.pi/2})
            sim.setObjectPosition(wallObjectHandle,sim_handle_parent,{-0.5+j*0.1,0.55-(i*0.1),0.08})
			sim.setObjectName(wallObjectHandle,"Vertical_block_".. tostring(j+1).."_"..tostring(i))
...
function deleteWalls()
	...
	for j=1,11,1 do
		for i=1,10,1 do
			if (sim.isHandleValid(horiwall[j][i]) == 1) then
				sim.removeObject(horiwall[j][i])
			end
			if (sim.isHandleValid(vertiwall[j][i]) == 1) then
				sim.removeObject(vertiwall[j][i])
			end
		end
	end
...
function createMaze()
	...
	--*******************************************************
	--               ADD YOUR CODE HERE
	--print("this is maze array in create maze \n",maze_array)
	for j=1,10,1 do
		for i=1,10,1 do
			if(maze_array[j][i] == 0)then
				if (sim.isHandleValid(horiwall[j][i]) == 1) then
					sim.removeObject(horiwall[j][i])
				end
				if (sim.isHandleValid(horiwall[j+1][i]) == 1) then
					sim.removeObject(horiwall[j+1][i])
				end
				if (sim.isHandleValid(vertiwall[i][j]) == 1) then
					sim.removeObject(vertiwall[i][j])
				end
				if (sim.isHandleValid(vertiwall[i+1][j]) == 1) then
					sim.removeObject(vertiwall[i+1][j])
				end
			elseif (maze_array[j][i] == 1) then
				if (sim.isHandleValid(horiwall[j][i]) == 1) then
					sim.removeObject(horiwall[j][i])
				end
				if (sim.isHandleValid(horiwall[j+1][i]) == 1) then
			...
			elseif (maze_array[j][i] == 7) then
				if (sim.isHandleValid(horiwall[j+1][i]) == 1) then
					sim.removeObject(horiwall[j+1][i])
				end
			elseif (maze_array[j][i] == 8) then
				if (sim.isHandleValid(horiwall[j][i]) == 1) then
					sim.removeObject(horiwall[j][i])
				end
				if (sim.isHandleValid(vertiwall[i][j]) == 1) then
			...
			elseif (maze_array[j][i] == 11) then
				if (sim.isHandleValid(vertiwall[i+1][j]) == 1) then
					sim.removeObject(vertiwall[i+1][j])
				end
			elseif (maze_array[j][i] == 12) then
				if (sim.isHandleValid(vertiwall[i][j]) == 1) then
					sim.removeObject(vertiwall[i][j])
				end
			elseif (maze_array[j][i] == 13) then
				if (sim.isHandleValid(horiwall[j][i]) == 1) then
					sim.removeObject(horiwall[j][i])
				end
			elseif (maze_array[j][i] == 14) then
				if (sim.isHandleValid(vertiwall[i][j]) == 1) then
					sim.removeObject(vertiwall[i][j])
				end
			end
	...
	--for t_1
	--for coordinate (0,4):
	if(sim.isHandleValid(horiwall[1][5])==1) then
		sim.removeObject(horiwall[1][5])
    end
    --for coordinate (9,5):
	if(sim.isHandleValid(horiwall[11][6])==1) then
		sim.removeObject(horiwall[11][6])
    end
    
	--for coordinate (4,9):
	if(sim.isHandleValid(vertiwall[11][5])==1) then
		sim.removeObject(vertiwall[11][5])
	end
...
function groupWalls()
	...
	for j=1,10,1 do
		for i=1,11,1 do
			if(sim.isHandleValid(vertiwall[i][j]) ==1 ) then
				table.insert( group, vertiwall[i][j])
				count = count + 1
			end
			
			if(sim.isHandleValid(horiwall[i][j]) ==1 ) then
				table.insert( group, horiwall[i][j])
				count = count + 1
			end
        end
	end
	
    wallsGroupHandle=sim.groupShapes(group,false)
	sim.setObjectName(wallsGroupHandle,'walls_t1_1')
	
	--print("wall has been gorupped")
...
function addToCollection()
	--*******************************************************
	--               ADD YOUR CODE HERE
	coll=sim.getCollectionHandle('colliding_objects')
  --[[
	if(coll == -1) then
		print('we got no handle collection')
	else
	...
	sim.addObjectToCollection(coll,wallsGroupHandle,sim_handle_single,0)
