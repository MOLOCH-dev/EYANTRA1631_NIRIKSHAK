...
--[[
# Team ID:			[ 1631 ]
# Author List:		[ Names of team members worked on this file separated by Comma: Dushant Panchbhai ]
# Filename:			task_2b
...
**************************************************************
	...
	---
	generateHorizo
position: coordinates of the object (x, y and z)ntalWalls()
**************************************************************	
]]--
function generateHorizontalWalls()
	...
	for j=0,10,1 do
		for i=1,10,1 do	
			...
			sim.setObjectPosition(horiwall[j+1][i],-1,{-0.55+(i*0.1),0.5-j*0.1,0.08})
			sim.setObjectParent(horiwall[j+1][i],parent,true)
...
function generateVerticalWalls()
	...
	for j=0,10,1 do
		for i=1,10,1 do
			...
			sim.setObjectOrientation(wallObjectHandle,-1,{0,0,math.pi/2})
			sim.setObjectPosition(wallObjectHandle,-1,{-0.5+j*0.1,0.55-(i*0.1),0.08})
