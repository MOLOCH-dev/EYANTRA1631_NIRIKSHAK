...
# Team ID:			[ 1631 ]
# Author List:		[ Dushant Panchbhai, Anushree Sabnis, Kushal Shah, Saurabh Powar ]
# Filename:			task_2b.py
# Functions:		init_remote_api_server, exit_remote_api_server, get_vision_sensor_image,
#					transform_vision_sensor_image, send_data, get_sim_dt
...
def get_vision_sensor_image():
	...
	##############	ADD YOUR CODE HERE	##############
	'''
	print("we are in get vision sensor image function")
	error,camhandle=sim.simxGetObjectHandle(client_id,'Vision_sensor',sim.simx_opmode_blocking)
	...
	if return_code == sim.simx_return_ok:
		...
		print("not got the image")
		print(image_resolution)
	print("we are done with visison sennsor image")
	'''

	#diff=51
	#print(client_id,"get")
	errorcode, camhandle = sim.simxGetObjectHandle(client_id,"Vision_sensor",sim.simx_opmode_oneshot_wait)
	#print(errorcode,camhandle,"handle")


	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id,camhandle,0,sim.simx_opmode_streaming)
	#print("rcode",return_code)
	simulation_time = sim.simxGetLastCmdTime(client_id)
	timep = sim.simxGetPingTime(client_id)
	#print(timep)
	...
	while (sim.simxGetConnectionId(client_id) != -1):
		...
		try:
			...
			simulation_time_2 = sim.simxGetLastCmdTime(client_id)
			#print("simtime",simulation_time)

			return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, camhandle, 0, sim.simx_opmode_buffer)
			#sim.simxGetVisionSensorImage(client_id,camhandle,0,sim.simx_opmode_discontinue)
...
def send_data(maze_array):
	...
	##############	ADD YOUR CODE HERE	##############
	for i in range(0,10):
		return_code,retInts,retFloats,retStrings,retBuffer=sim.simxCallScriptFunction(client_id,'Base',2050006,'receiveData',maze_array[i],inputFloats,inputStrings,inputBuffer,sim.simx_opmode_blocking)
	...
	##############################n####################
		...
		#print('try again')
	#else:
		#print('something has assigned in return code')
